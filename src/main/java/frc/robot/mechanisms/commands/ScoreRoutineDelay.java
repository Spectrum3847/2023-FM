...
public class ScoreRoutineDelay extends CommandBase {
    ...
    /**
     * Scoring routine delay for cones and cubes. If score routine button is pressed before shoulder
     * is in scoring position, command will wait for the proper time or timeout with a set maximum:
     * {@link #maxTimeout} This command must be used along with the scoring command. Proper usage:
     * new ScoreRoutineDelay(ScoreType.CONE).andThen(coneScoreRoutine());
     *
    ...
    public boolean isFinished() {
        return shouldEnd || Timer.getFPGATimestamp() - startTime >= maxTimeout;
    }
